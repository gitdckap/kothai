...
public class VoiceRecorder {
    ...
    private static final int MAX_SPEECH_LENGTH_MILLIS = 300 * 1000;
    ...
    private AudioRecord createAudioRecord() {
        final AudioRecord audioRecord = new AudioRecord(MediaRecorder.AudioSource.MIC,
                16000, CHANNEL, ENCODING, AudioRecord.getMinBufferSize(16000, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT));

        if (NoiseSuppressor.isAvailable()) {
            NoiseSuppressor noiseSuppressor = NoiseSuppressor.create(audioRecord.getAudioSessionId());
            if (noiseSuppressor != null) {
                noiseSuppressor.setEnabled(true);
                System.out.println("✅ Noise Suppression Enabled!");
            }
        }
        ...
        if (AcousticEchoCanceler.isAvailable()) {
            ...
            if (echoCanceler != null) {
                ...
                System.out.println("✅ Acoustic Echo Cancellation Enabled!");
            } else {
                System.out.println("❌ Failed to enable Acoustic Echo Canceler.");
            }
        }

        if (audioRecord.getState() == AudioRecord.STATE_INITIALIZED) {
            mBuffer = new byte[(16000 / 1000) * 250 * 2];
            return audioRecord;
        } else {
            audioRecord.release();
        }
...
//            }
//            final AudioRecord audioRecord = new AudioRecord(MediaRecorder.AudioSource.MIC,
//                    sampleRate, CHANNEL, ENCODING, 16000 * 2 / 2);
//            if (audioRecord.getState() == AudioRecord.STATE_INITIALIZED) {
//                mBuffer = new byte[16000 * 2 / 2];
//                return audioRecord;
//            } else {
//                audioRecord.release();
//            }
        ...
        public void run() {
            while (true) {
                synchronized (mLock) {
                    ...
                    final int size = mAudioRecord.read(mBuffer, 0, (16000 / 1000) * 250 * 2);
